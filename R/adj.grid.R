...
adj.grid <- function(coords, max.dist = NULL, radius = 1, spam = F, type = "radius"){
  
  if(!(type %in% c("radius", "dist", "compass"))) stop('Invalid neighborhood type. Choose either "dist" or "compass".')
  ...
  if(type == "radius"){
    ...
    adj <- apply(coords, 1, function(pt)
      sqrt((as.numeric(pt["y"]) - as.numeric(coords[,'y']))^2  + (as.numeric(pt["x"]) - as.numeric(coords[,"x"]))^2) <= max.dist)
